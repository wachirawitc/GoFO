...
4. **Rewrite.** The original CSS is reproduced verbatim except for the `url()` values. Query strings and fragments are preserved — `?#iefix` for IE8 and `#FontName` for SVG fonts — so legacy formats keep working.
